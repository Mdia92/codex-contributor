...
GitHub Issue URL / repository URL
      ...
      └──────────┬──────────┘
                 ▼
      ┌─────────────────────┐
      │ Investigation       │  GPT-5.6 + cited evidence + confidence gate
      └──────────┬──────────┘
                 ▼
      ...
      └──────────┬──────────┘
                 ▼
      ┌─────────────────────┐
      │ Code writer         │  one cached model call per file
      └──────────┬──────────┘
                 ▼
      ┌─────────────────────┐
      │ Validation loop     │  tests → failure repair, max 5 iterations
      └──────────┬──────────┘
                 ▼
      ┌─────────────────────┐
...
3. **Investigation** sends the issue and line-numbered repository excerpts to GPT-5.6 Sol. The prompt requires file-level evidence for every codebase claim. Its structured review is cached and validated.
4. **Confidence gate** stops implementation below 0.50 and emits a Human Review Required section.
5. **Implementation planning** converts the review into a minimal file/change/test plan.
6. **Code writing** makes one complete-file model call per planned file, caches it, and rejects paths outside the working copy.
7. **Engineering validation** runs the detected framework. Only failures invoke repair calls, and the loop stops after five iterations.
